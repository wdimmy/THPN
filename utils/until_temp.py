...
def entityList(kb_path, task_id):
    type_dict = get_type_dict(kb_path, dstc2=(task_id == 'dstc2'))
    entity_list = []
...
def prepare_data_seq(task, batch_size=100, shuffle=True):
    ...
    can, ind2cand = candid2DL('data/dialog-bAbI-tasks/dialog-babi-candidates.txt',
                              'data/dialog-bAbI-tasks/dialog-babi-kb-all.txt', int(task))
