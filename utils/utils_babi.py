...
def prepare_data_seq(task,batch_size=100,shuffle=True):
    file_train = 'data/babi/dialog-babi-task{}trn.txt'.format(task)
    file_dev = 'data/babi/dialog-babi-task{}dev.txt'.format(task)
    file_test = 'data/babi/dialog-babi-task{}tst.txt'.format(task)
    if (int(task) != 6):
        file_test_OOV = 'data/babi/dialog-babi-task{}tst-OOV.txt'.format(task)
