...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        index_s = self.preprocess_target(index_s, src_seq)
    ...
    def preprocess_inde(self, sequence, src_seq):
        ...
        return sequence
    def preprocess_target(self,sequence,src_seq):
        ...
        for line in fin:
            ...
            if line:
                ...
                if '\t' in line:
                    ...
                    r = line
                    if USEKB:
                        contex_arr += generate_memory(r, "kb", "")
                        conversation_arr += generate_memory(r, "kb", "")
                    else:
                        conversation_arr += generate_memory(r, "kb", "")
            else:  # 另外一段对话
                cnt_lin += 1
...
def get_seq(pairs, lang, batch_size, type, max_len,max_r_ans):
    ...
    for pair in pairs:
        ...
        entity_wet.append(pair[8])
        entity_cal.append(pair[9])
        entity_nav.append(pair[10])
        if (type):
...
def prepare_data_seq(task, batch_size=100, shuffle=True):
    # test sample
    file_train = 'data/kvr/kvrtrain.txt'
    file_dev = 'data/kvr/kvrdev.txt'
    file_test = 'data/kvr/kvrtest.txt'
    qa_dict={}
    qa_dict_trn={}
    if args['dataset'] == 'kvr':
        qa_dict = read_lines(
            '/home/wdl/Code/czy/MNAG/data/ir_data/kvr/kvr-1/tstQuestions.txt',
            '/home/wdl/Code/czy/MyTest/kvr_{}_best/ir_tst_n_best.txt'.format(TOP_K))
        qa_dict_trn = read_lines(
            '/home/wdl/Code/czy/MNAG/data/ir_data/kvr/kvr-1/trnQuestions.txt',
            '/home/wdl/Code/czy/MyTest/kvr_{}_best/ir_trn_n_best.txt'.format(TOP_K))
    else:
        print('IR data path error')
    ent=read_kb_list("data/kvr/kvr_kb.txt") 
